...
class BinarySemaphore {
    SemaphoreHandle_t _handle;
public:
    BinarySemaphore() : _handle(xSemaphoreCreateBinary()) {}
    ~BinarySemaphore() { if (_handle) vSemaphoreDelete(_handle); }
    ...
    bool giveFromISR(BaseType_t* pxHigherPriorityTaskWoken = nullptr) {
        ...
        if (!pxHigherPriorityTaskWoken) pxHigherPriorityTaskWoken = &dummy;
...
public:
    Queue() : _handle(xQueueCreate(N, sizeof(T))) {}
    ~Queue() { if (_handle) vQueueDelete(_handle); }
...
public:
    Task(const char* name, uint16_t stackDepth, UBaseType_t priority,
         void (*fn)(void*), void* param = nullptr) {
