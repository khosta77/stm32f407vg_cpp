...
class Mpu6050 : public IImu {
    ...
    bool writeReg(uint8_t reg, uint8_t val) {
        return _i2c.writeReg(_addr, reg, std::span<const uint8_t>(&val, 1)) == driver::Status::Ok;
    }

    bool readRegs(uint8_t reg, uint8_t* buf, size_t len) {
        return _i2c.readReg(_addr, reg, std::span<uint8_t>(buf, len)) == driver::Status::Ok;
    }
...
public:
    ...
    bool init() override {
        ...
        for (uint32_t i = 0; i < 100000; ++i) { __asm volatile("nop"); }
    ...
    void setAccelRange(uint8_t g) override {
        uint8_t val;
        switch (g) {
            case 4:  val = 0x08; _accelScale = 8192.0f;  break;
            case 8:  val = 0x10; _accelScale = 4096.0f;  break;
            case 16: val = 0x18; _accelScale = 2048.0f;  break;
            default: val = 0x00; _accelScale = 16384.0f; break;
        }
    ...
    void setGyroRange(uint16_t dps) override {
        uint8_t val;
        switch (dps) {
            case 500:  val = 0x08; _gyroScale = 65.5f;  break;
            case 1000: val = 0x10; _gyroScale = 32.8f;  break;
            case 2000: val = 0x18; _gyroScale = 16.4f;  break;
            default:   val = 0x00; _gyroScale = 131.0f;  break;
        }
